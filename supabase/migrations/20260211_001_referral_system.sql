...
-- Add referral fields to users table (IF NOT EXISTS for idempotency)
...
ALTER TABLE users ADD COLUMN IF NOT EXISTS last_code_generation TIMESTAMPTZ;
ALTER TABLE users ADD COLUMN IF NOT EXISTS codes_generated_this_period INTEGER DEFAULT 0;
ALTER TABLE users ADD COLUMN IF NOT EXISTS stripe_customer_id TEXT;
ALTER TABLE users ADD COLUMN IF NOT EXISTS stripe_subscription_id TEXT;
ALTER TABLE users ADD COLUMN IF NOT EXISTS subscription_status TEXT CHECK (subscription_status IN ('trial', 'active', 'past_due', 'canceled', 'inactive'));

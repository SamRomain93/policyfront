    ...
    <div className="min-h-screen bg-cream-50">
      ...
      <aside className="fixed top-0 left-0 w-64 h-full bg-near-black text-cream-50 p-6 flex flex-col z-40 hidden lg:flex">
        ...
        <nav className="flex-1 space-y-1">
          {[
            ...
            <Link
              key={item.label}
              href={item.href}
              className="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm text-cream-200 hover:text-cream-50 hover:bg-white/10 transition"
            >
        ...
        <div className="border-t border-white/10 pt-4 mt-4">
          <div className="text-xs text-cream-200/60">PolicyFront v0.1</div>
        </div>
      </aside>
